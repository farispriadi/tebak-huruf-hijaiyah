...
from PySide2.QtGui import QIcon, QKeySequence, QBrush, QPen, QPixmap,QStandardItemModel, QStandardItem, QColor, QPainter,QTextOption, QImage, QPalette,QPainterPath, QRegion, QFont
from PySide2.QtCore import Qt, QStringListModel,QAbstractListModel,QRectF,QIODevice,QDataStream,QByteArray,Signal, Slot,QThread, QSize
import random
import time
...
def random_soal(current_huruf="ya"):
    ...
    DB_ANSWER = [random.sample(DB_HURUF,2) for i in range(5)]

    print("DB_ANSWER",DB_ANSWER)
    DB_OPTIONS = [val+random.sample([ hrf for hrf in DB_HURUF if hrf not in val],3) for val in  DB_ANSWER]
...
class NewDialog(QDialog):
    ...
    def __init__(self, main_window,icon=None,parent=None):
        ...
        self.setWindowTitle("Profil Pemain")
        ...
        self.age.setFont(QFont('Arial', 30))
        self.level = QSpinBox()
        ...
        self.level.setFont(QFont('Arial', 30))

        self.huruf = QLabel()
        self.current_huruf = self.main_window.current_huruf
        # self.huruf.value=self.main_window.current_huruf
        # path = IMAGES_PATH+"{}.png".format(self.huruf.value)
        # pixmap = QPixmap(QImage(path))
        # self.huruf.setPixmap(pixmap)
        # self.huruf.setFixedHeight(100)
        # self.huruf.setFixedWidth(150)
        ...
        self.age_label.setFont(QFont('Arial', 30))
        self.level_label = QLabel("Jilid")
        self.level_label.setFont(QFont('Arial', 30))
        self.huruf_label = QLabel("Batas Huruf")
        self.huruf_label.setFont(QFont('Arial', 30))
...
class ImageItem(QGraphicsRectItem):
        ...
        if (event.mimeData().hasFormat("application/x-qabstractitemmodeldatalist")):
            ...
            if self.main_window.DB_OPTIONS[self.page][row] == self.value:
                if row>=-1:
                    ...
                    path = IMAGES_PATH+self.main_window.DB_OPTIONS[self.page][row]+"-ans.png"
    ...
    def paint(self, painter,option, widget):
        ...
        painter.drawText(self.boundingRect(),self.m_text,
...
class NextWidget(QWidget):
    ...
    def hideOverlay(self):
        ...
        self.parent().parent().item_gameover.setHtml("")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, screen_geo = None):
        ...
        self.current_page = 0
        self.skor = 0
        self.answer_correct = [False,False]
        ...
        if not is_correct:
            if self.heart_status >=1:
                ...
                if self.heart_status == 0:
                    print("GAME OVER")
                    self.item_gameover.setHtml("<html><h1 style='text-align:center;'>GAME OVER</h1></html>")
            ...
            if all(self.answer_correct):
                ...
                if self.current_page==self.total_pages-1:
                    self.item_gameover.setHtml("<html><h1 style='text-align:center;'>KAMU MENANG!</h1></html>")
                    ...
                    self.item_gameover.setHtml("<html><h1 style='text-align:center;'>MASYA ALLAH!</h1></html>")
                    item = self.check_images[self.current_page]
    ...
    def resetGame(self):
        ...
        self.item_gameover.setHtml("")
        for item in self.heart_images:
            path = IMAGES_PATH+"heart.png"
            pixmap = QPixmap(QImage(path))
            item.setPixmap(pixmap)
    ...
    def createGraphicsItems(self,current_page=0,total_pages=5,level=1,variant=0):
        ...
        # self.item_question = QGraphicsTextItem()
        # self.item_question.setHtml("<h1>SOAL {} / {}</h1>".format(current_page,total_pages))
        # self.item_question.setPos(800,10)
        # self.scene.addItem(self.item_question)


        self.item1 = ImageItem(mw=self)
        self.scene.addItem(self.item1)
        self.item1.setRect(huruf_box)
        self.item1.setPos(300,150)
        self.item1.setBrush(QColor("#FFFFFF"))
        path1 = IMAGES_PATH+"tanya.png"
        self.image1 = QBrush(QImage(path1))
        self.item1.setBrush(self.image1)
        ...
        self.item2.index = 0
        # self.item2.corect_ans_signal.connect(self.getAnswer)

        self.item3 = ImageItem()
        self.item3.setAcceptDrops(False)
        self.scene.addItem(self.item3)
        self.item3.setRect(kalimat_box)
        self.item3.setPos(300,350)
        self.item3.setBrush(QColor("#D9DDDC"))
        # path3 = IMAGES_PATH+"alif-ba-ans.png"
        # self.image3 = QBrush(QImage(path3))
        # self.item3.setBrush(self.image3)
        ...
        # self.item4 = QGraphicsPixmapItem()
        # self.item4.setAcceptDrops(False)
        # self.scene.addItem(self.item4)
        # self.item4.setPos(310,350)
        # path4 = IMAGES_PATH+"alif.png"
        # self.image4 = QPixmap(QImage(path4))
        # self.item4.setPixmap(self.image4)

        # self.item5 = QGraphicsPixmapItem()
        # self.item5.setAcceptDrops(False)
        # self.scene.addItem(self.item5)
        # self.item5.setPos(410,350)
        # path5 = IMAGES_PATH+"alif.png"
        # self.image5 = QPixmap(QImage(path5))
        # self.item5.setPixmap(self.image5)

        # self.item6 = QGraphicsPixmapItem()
        # self.item6.setAcceptDrops(False)
        # self.scene.addItem(self.item6)
        # self.item6.setPos(510,350)
        # path6 = IMAGES_PATH+"alif.png"
        # self.image6 = QPixmap(QImage(path6))
        # self.item6.setPixmap(self.image6)

        # self.item7 = QGraphicsPixmapItem()
        # self.item7.setAcceptDrops(False)
        # self.scene.addItem(self.item7)
        # self.item7.setPos(610,350)
        # path7 = IMAGES_PATH+"alif.png"
        # self.image7 = QPixmap(QImage(path7))
        # self.item7.setPixmap(self.image7)
